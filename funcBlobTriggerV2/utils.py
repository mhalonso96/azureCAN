...
def setup_fs(s3, key="", secret="", endpoint="", region="",cert="", passwords={}):
    ...
    if s3:
        ...
        import etncanedge_files

        base_path = Path(__file__).parent
        fs = etncanedge_files.LocalFileSystem(base_path=base_path, passwords=passwords)
...
def load_dbc_files(dbc_paths):
    ...
    for dbc in dbc_paths:
        # print(f'Diretorio do {dbc}')
        # dir_exe =  Path(__file__).resolve().parent
        # print ("Estou load_dbc_files: ", dbc)
        # dbcs = dir_exe / dbc
...
def list_log_files(fs, devices, start_times, verbose=True, passwords={}):
    ...
    if len(start_times):
        for idx, device in enumerate(devices):
            ...
            log_files_device = etncanedge_files.get_log_files(fs, [device], start_date=start, passwords=passwords)
...
def restructure_data(df_phys, res, filter_signals, filter_gps, ffill=False):
    ...
    if not df_phys.empty and res != "":
        ...
        columns_to_complete = df_phys.columns.difference(['Altitude','Latitude', 'Longitude'])
        df_phys[columns_to_complete] = df_phys[columns_to_complete].fillna(0)
        for column in filter_gps:
            df_phys[column]=df_phys[column].bfill()
        # for column in ['TransSelectedGear', 'TransCurrentGear']:
        #     df_phys[column]=df_phys[column].round(0)
        df_phys = df_phys[filter_signals]
        # df_phys.index = range(len(df_phys))
        # df_phys.index.name = 'Time'
    if ffill:
